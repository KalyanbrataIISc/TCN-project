#!/usr/bin/env python3
# angle_response_snn_noise.py — plot SNN firing‐rates vs. line angle w/ background noise
...
MAX_RATE_HZ   = 300.                   # peak Poisson for signal
BIAS_SCALE    = 0.3                    # bias→mV
NOISE_RATE_HZ = 10.0                   # background spike rate (Hz)
NOISE_WEIGHT  = 0.05                   # mV added per noise spike
OUT_FIG       = PLOTS / "snn_angle_response_noise.png"
...
def main():
    ...
    for flat in X:
        ...
        # create neuron groups: h1,h2,h3,out
        ...
        # ─── NEW: background noise into every layer ───────────────────
        for G in groups:
            Ng = PoissonGroup(len(G), rates=NOISE_RATE_HZ*Hz)
            Sn = Synapses(Ng, G, on_pre=f'I_syn_post += {NOISE_WEIGHT}*mV')
            Sn.connect(j='i')  # one‐to‐one noise
            net.add(Ng, Sn)
    ...
    plt.title(f"SNN response vs. line angle\n"
              f"(noise: {NOISE_RATE_HZ:.1f} Hz, w={NOISE_WEIGHT} mV)")
    plt.legend(loc="best")
    plt.grid(alpha=0.3)
    plt.tight_layout()
