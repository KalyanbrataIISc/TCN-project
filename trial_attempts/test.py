#!/usr/bin/env python3
# test.py — end-to-end ANN → SNN on H/V lines
...
N_PER_CLASS = 200      # 200 horizontals + 200 verticals
...
Ws = [layer.get_weights()[0].astype(np.float32)
      for layer in ann.layers if len(layer.get_weights())==2]
bs = [layer.get_weights()[1].astype(np.float32)
      for layer in ann.layers if len(layer.get_weights())==2]
...
for idx in range(len(X_test)):
    flat = X_test[idx]        # [0..1]
    # dark pixels → high rate
    rates = (1.0 - flat) * MAX_RATE_HZ

    # create neuron groups
    sizes = [Ws[0].shape[1], Ws[1].shape[1],
             Ws[2].shape[1], Ws[3].shape[1]]
    groups = []
    for n in sizes:
    ...
    # inject constant bias I_bias (scaled ANN b)
    for G,b in zip(groups, bs):
        b0 = (b * BIAS_SCALE)
        # pad/truncate to match G.N
        b0 = np.pad(b0, (0,max(0,len(G)-len(b0))),
                    'constant')[:len(G)]
        G.I_bias = b0 * mV
...
# ANN scatter
plt.subplot(1,2,1)
c = angles_test/90.  # 0=horiz→0, 1=vert→1
plt.scatter(ann_out[:,0], ann_out[:,1],
            c=c, cmap='coolwarm', s=50)
...
# SNN scatter
plt.subplot(1,2,2)
plt.scatter(snn_rates[:,1], snn_rates[:,0],
            c=c, cmap='coolwarm', s=50)
